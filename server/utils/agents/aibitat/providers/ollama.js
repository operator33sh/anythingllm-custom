...
// Module-level cache: avoids a live Ollama API call on every message in automatic mode.
// Key: model name, Value: boolean (supports tool calling)
...
class OllamaProvider extends InheritMultiple([Provider, UnTooled]) {
  ...
  async supportsNativeToolCalling() {
    ...
    if (_toolCapabilityCache.has(this.model))
      return _toolCapabilityCache.get(this.model);
    const ollama = new OllamaAILLM(null, this.model);
    const capabilities = await ollama.getModelCapabilities();
    // "unknown" means the API call failed — don't cache, assume tools are supported
    // so the agent handler is invoked and native tool calling is attempted.
    if (capabilities.tools === "unknown") return true;
    const result = capabilities.tools === true;
    _toolCapabilityCache.set(this.model, result);
    this._supportsToolCalling = result;
    return result;
  }
