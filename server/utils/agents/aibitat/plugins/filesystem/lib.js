...
class FilesystemManager {
   ...
   */
  isToolAvailable() {
    if (process.env.AGENT_FILESYSTEM_ENABLED === "true") return true;
  ...
  async #initializeFilesystem(directories = null) {
    const envDirs = process.env.AGENT_FILESYSTEM_ALLOWED_DIRS
      ? process.env.AGENT_FILESYSTEM_ALLOWED_DIRS.split(",")
          .map((d) => d.trim())
          .filter(Boolean)
      : [];
    const combinedDirs = [...(directories || []), ...envDirs];

    if (combinedDirs.length > 0) {
      this.#allowedDirectories = combinedDirs.map((dir) =>
    ...
    console.log("[FilesystemManager] Allowed directories:", this.#allowedDirectories);
