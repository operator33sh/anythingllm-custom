...
export default function ChatContainer({
  ...
  useEffect(() => {
    async function fetchReply() {
      ...
      const _ts = `[${_pad(_now.getDate())}/${_pad(_now.getMonth() + 1)}/${_now.getFullYear()} ${_pad(_now.getHours())}:${_pad(_now.getMinutes())}]`;
      const _timedPrompt = `${_ts} ${promptMessage.userMessage}`;
